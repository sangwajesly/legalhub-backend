...
from app.models.lawyer import firestore_lawyer_to_model, lawyer_model_to_firestore, Lawyer
...
async def list_lawyers(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100)):
	docs, total = await firebase_service.query_collection("lawyers", filters={}, limit=page_size, offset=(page - 1) * page_size)
	lawyers = []
	for doc_id, doc in docs:
		try:
			...
			continue
	return LawyerListResponse(lawyers=lawyers, total=total, page=page, pageSize=page_size)
...
async def create_or_update_lawyer(data: LawyerCreate, current_user: Optional[dict] = Depends(get_current_user)):
	if not current_user:
	...
	return LawyerProfile.model_validate(firestore_lawyer_to_model(doc, uid).model_dump())


@router.put("/{lawyer_id}", response_model=LawyerProfile)
async def update_lawyer(lawyer_id: str, data: LawyerUpdate, current_user: Optional[dict] = Depends(get_current_user)):
	if not current_user:
		raise HTTPException(status_code=401, detail="Authentication required")
	...
	return LawyerProfile.model_validate(firestore_lawyer_to_model(updated, lawyer_id).model_dump())


@router.delete("/{lawyer_id}")
async def delete_lawyer(lawyer_id: str, current_user: Optional[dict] = Depends(get_current_user)):
	if not current_user:
		raise HTTPException(status_code=401, detail="Authentication required")
	is_admin = current_user.get("is_admin")
	if current_user.get("uid") != lawyer_id and not is_admin:
