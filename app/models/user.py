...
class UserProfile(BaseModel):
    ...
    # Additional metadata
    interests: list[str] = Field(default_factory=list, description="Legal topics of interest")
...
class UserInDB(User):
    ...
    refresh_token: Optional[str] = Field(default=None, description="Current refresh token")
    password_reset_token: Optional[str] = Field(default=None, description="Password reset token")
    password_reset_expires: Optional[datetime] = Field(default=None, description="Reset token expiration")
    failed_login_attempts: int = Field(default=0, description="Count of failed login attempts")
...
class UserStats(BaseModel):
    ...
    total_articles_written: int = Field(default=0, description="Articles written (for lawyers)")
...
def firestore_user_to_model(doc_data: dict, uid: str) -> User:
    ...
    return User(
        ...
        email_verified=doc_data.get("emailVerified", doc_data.get("email_verified", False)),
        phone_number=doc_data.get("phoneNumber", doc_data.get("phone_number")),
        photo_url=doc_data.get("photoURL", doc_data.get("photo_url")),
